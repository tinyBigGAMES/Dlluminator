...
type
  ...
  { TDlmCallback }
  TDlmCallback<T> = record
    Callback: T;
    UserData: Pointer;
    function IsAssigned(): Boolean;
  end;

  { TDlmCaptureConsoleCallback }
  TDlmCaptureConsoleCallback = reference to procedure(const ALine: string; const AUserData: Pointer);

  { TDlmStatusCallback }
  TDlmStatusCallback = reference to procedure(const AText: string; const AUserData: Pointer);

  { TDlmVersionInfo }
  TDlmVersionInfo = record
    Major: Word;
    Minor: Word;
    Patch: Word;
    Build: Word;
  ...
  public
    ...
    class procedure CaptureConsoleOutput(const ATitle: string; const ACommand: PChar; const AParameters: PChar; const AWorkDir: string; var AExitCode: DWORD; const AUserData: Pointer; const ACallback: TDlmCaptureConsoleCallback); static;
    class procedure CaptureZigConsolePTY(const ACommand: PChar; const AParameters: PChar; const AWorkDir: string; var AExitCode: DWORD; const AUserData: Pointer; const ACallback: TDlmCaptureConsoleCallback); static;
    ...
    class function  GetVersionInfo(out AVersionInfo: TDlmVersionInfo; const AFilePath: string = ''): Boolean; static;
  ...
  { TDlmErrorRelated }
  TDlmErrorRelated = record
    Range: TDlmSourceRange;
    Message: string;
  end;
  ...
  TDlmError = record
    Range: TDlmSourceRange;
    Severity: TDlmErrorSeverity;
    Code: string;
    Message: string;
    Related: TArray<TDlmErrorRelated>;
  ...
  public
    ...
    procedure Add(
      const ARange: TDlmSourceRange;
      const ASeverity: TDlmErrorSeverity;
      const ACode: string;
    ...
    function GetItems(): TList<TDlmError>;
  ...
  { TDlmBaseObject }
  TDlmBaseObject = class
  protected
    FErrors: TDlmErrors;
    FOwnsErrors: Boolean;
    FStatusCallback: TDlmCallback<TDlmStatusCallback>;
  public
    constructor Create(); virtual;
    destructor Destroy(); override;
    ...
    function  GetStatusCallback(): TDlmStatusCallback;
    procedure SetStatusCallback(const ACallback: TDlmStatusCallback; const AUserData: Pointer = nil); virtual;
    procedure SetErrors(const AErrors: TDlmErrors); virtual;
    function  GetErrors(): TDlmErrors;
    procedure PropagateErrors(const AChild: TDlmBaseObject);
  end;

  { TDlmCommandBuilder }
  TDlmCommandBuilder = class(TDlmBaseObject)
  private
    FParams: TStringList;
  public
...
function TDlmCallback<T>.IsAssigned(): Boolean;
begin
...
class function TDlmUtils.EnableVirtualTerminalProcessing(): Boolean;
var
  HOut: THandle;
  LMode: DWORD;
begin
...
class procedure TDlmUtils.InitConsole();
begin
...
class procedure TDlmUtils.FailIf(const Cond: Boolean; const Msg: string; const AArgs: array of const);
begin
  if Cond then
    raise Exception.CreateFmt(Msg, AArgs);
end;

class function TDlmUtils.GetTickCount(): DWORD;
begin
  Result := WinApi.Windows.GetTickCount();
end;

class function TDlmUtils.GetTickCount64(): UInt64;
begin
  Result := WinApi.Windows.GetTickCount64();
end;

class function TDlmUtils.HasConsole(): Boolean;
begin
  Result := Boolean(GetConsoleWindow() <> 0);
end;

class procedure TDlmUtils.ClearToEOL();
begin
  if not HasConsole() then Exit;
  Write(#27'[0K');
end;

class procedure TDlmUtils.Print();
begin
  Print('');
end;

class procedure TDlmUtils.PrintLn();
begin
  PrintLn('');
end;

class procedure TDlmUtils.Print(const AText: string);
begin
  if not HasConsole() then Exit;
  Write(AText + COLOR_RESET);
end;

class procedure TDlmUtils.Print(const AText: string; const AArgs: array of const);
begin
  if not HasConsole() then Exit;
  Write(Format(AText, AArgs) + COLOR_RESET);
end;

class procedure TDlmUtils.PrintLn(const AText: string);
begin
  if not HasConsole() then Exit;
  WriteLn(AText + COLOR_RESET);
end;

class procedure TDlmUtils.PrintLn(const AText: string; const AArgs: array of const);
begin
  if not HasConsole() then Exit;
  WriteLn(Format(AText, AArgs) + COLOR_RESET);
end;

class function TDlmUtils.RGB(const AR, AG, AB: Byte): string;
begin
  Result := #27'[38;2;' + IntToStr(AR) + ';' + IntToStr(AG) + ';' + IntToStr(AB) + 'm';
end;

class function TDlmUtils.Pause(const AMsg, AQuit: string): Boolean;
var
  LInput: string;
begin
  Result := False;
...
class function TDlmUtils.AsUTF8(const AValue: string; ALength: PCardinal): Pointer;
begin
...
class function TDlmUtils.ToAnsi(const AValue: string): AnsiString;
var
  LBytes: TBytes;
begin
...
class procedure TDlmUtils.ProcessMessages();
var
  LMsg: TMsg;
begin
...
class function TDlmUtils.RunPE(const AExe, AParams, AWorkDir: string; const AWait: Boolean; const AShowCmd: Word): Cardinal;
var
  LAppPath: string;
  LCmd: UnicodeString;
  LSI: STARTUPINFOW;
...
class function TDlmUtils.WindowsPathToWSL(const APath: string): string;
var
  LFullPath: string;
  LDrive: Char;
begin
...
class function TDlmUtils.RunElf(const AElf, AWorkDir: string): Cardinal;
var
  LWslPath: string;
  LCmd: UnicodeString;
  LSI: STARTUPINFOW;
...
class procedure TDlmUtils.CaptureConsoleOutput(const ATitle: string; const ACommand: PChar; const AParameters: PChar; const AWorkDir: string; var AExitCode: DWORD; const AUserData: Pointer; const ACallback: TDlmCaptureConsoleCallback);
const
  CReadBuffer = 1024 * 2;
var
...
class procedure TDlmUtils.CaptureZigConsolePTY(const ACommand: PChar; const AParameters: PChar; const AWorkDir: string; var AExitCode: DWORD; const AUserData: Pointer; const ACallback: TDlmCaptureConsoleCallback);
const
  CReadBuffer = 4096;
var
...
class function TDlmUtils.CreateProcessWithPipes(const AExe, AParams, AWorkDir: string; out AStdinWrite: THandle; out AStdoutRead: THandle; out AProcessHandle: THandle; out AThreadHandle: THandle): Boolean;
var
...
class function TDlmUtils.CreateDirInPath(const AFilename: string): Boolean;
var
  LPath: string;
begin
...
class procedure TDlmUtils.CopyFilePreservingEncoding(const ASourceFile, ADestFile: string);
var
  LSourceBytes: TBytes;
begin
...
class function TDlmUtils.DetectFileEncoding(const AFilePath: string): TEncoding;
var
  LBytes: TBytes;
  LEncoding: TEncoding;
begin
...
class function TDlmUtils.EnsureBOM(const AText: string): string;
const
  UTF16_BOM = #$FEFF;
begin
  Result := AText;
...
class function TDlmUtils.EscapeString(const AText: string): string;
var
  LI: Integer;
  LChar: Char;
  LNextChar: Char;
...
class function TDlmUtils.StripAnsi(const AText: string): string;
var
...
class function TDlmUtils.ExtractAnsiCodes(const AText: string): string;
var
  LResult: TStringBuilder;
  LIdx: Integer;
  LLen: Integer;
...
class function TDlmUtils.GetVersionInfo(out AVersionInfo: TDlmVersionInfo; const AFilePath: string): Boolean;
var
  LFileName: string;
  LInfoSize: DWORD;
  LHandle: DWORD;
...
class function TDlmUtils.IsValidWin64PE(const AFilePath: string): Boolean;
var
  LFile: TFileStream;
...
class procedure TDlmUtils.UpdateIconResource(const AExeFilePath, AIconFilePath: string);
type
...
class procedure TDlmUtils.UpdateVersionInfoResource(const PEFilePath: string; const AMajor, AMinor, APatch: Word; const AProductName, ADescription, AFilename, ACompanyName, ACopyright: string; const AURL: string);
type
...
class function TDlmUtils.ResourceExist(const AResName: string): Boolean;
begin
  Result := Boolean((FindResource(HInstance, PChar(AResName), RT_RCDATA) <> 0));
end;

class function TDlmUtils.AddResManifestFromResource(const AResName: string; const AModuleFile: string; ALanguage: Integer): Boolean;
var
  LHandle: THandle;
...
class procedure TDlmUtils.UpdateRCDataResource(const AExeFilePath: string;
...
class function TDlmUtils.GetFileSHA256(const APath: string): string;
begin
  Result := THashSHA2.GetHashStringFromFile(APath).ToLower();
end;

class function TDlmUtils.GetRelativePath(const ABasePath, AFullPath: string): string;
var
  LBasePath: string;
  LFullPath: string;
  LBaseLen: Integer;
...
class function TDlmUtils.NormalizePath(const APath: string): string;
begin
  Result := APath.Replace(PathDelim, '/');
end;

class function TDlmUtils.DisplayPath(const APath: string): string;
begin
  Result := TPath.GetFullPath(APath).Replace('\', '/');
end;

class function TDlmUtils.GetEnv(const AName: string): string;
begin
  Result := GetEnvironmentVariable(AName);
end;

class procedure TDlmUtils.SetEnv(const AName: string; const AValue: string);
begin
  SetEnvironmentVariable(PChar(AName), PChar(AValue));
end;

class function TDlmUtils.HasEnv(const AName: string): Boolean;
begin
  Result := not GetEnv(AName).IsEmpty();
end;

class function TDlmUtils.RunFromIDE(): Boolean;
begin
  Result := HasEnv('BDS');
end;

class function TDlmUtils.CountLines(
...
{ TDlmSourceRange }
procedure TDlmSourceRange.Clear();
begin
  Filename := '';
  StartLine := 0;
  StartColumn := 0;
...
function TDlmSourceRange.IsEmpty(): Boolean;
begin
  Result := (StartLine = 0) and (StartColumn = 0);
end;

function TDlmSourceRange.ToPointString(): string;
begin
  if IsEmpty() then
    Result := ''
  else
...
function TDlmSourceRange.ToRangeString(): string;
begin
  if IsEmpty() then
    Result := ''
...
function TDlmError.GetSeverityString(): string;
begin
  case Severity of
...
function TDlmError.ToIDEString(): string;
begin
...
function TDlmError.ToFullString(): string;
var
...
constructor TDlmErrors.Create();
begin
  inherited;

  FItems := TList<TDlmError>.Create();
  FMaxErrors := DEFAULT_MAX_ERRORS;
end;

destructor TDlmErrors.Destroy();
begin
  FItems.Free();
...
function TDlmErrors.CountErrors(): Integer;
var
  LError: TDlmError;
begin
  Result := 0;
...
procedure TDlmErrors.AddRelated(
  const ARange: TDlmSourceRange;
  const AMessage: string
);
var
  LError: TDlmError;
  LRelated: TDlmErrorRelated;
  LLen: Integer;
begin
...
function TDlmErrors.HasHints(): Boolean;
var
  LError: TDlmError;
begin
  Result := False;
...
function TDlmErrors.HasWarnings(): Boolean;
var
  LError: TDlmError;
begin
  Result := False;
...
function TDlmErrors.HasErrors(): Boolean;
var
  LError: TDlmError;
begin
  Result := False;
...
function TDlmErrors.HasFatal(): Boolean;
var
  LError: TDlmError;
begin
  Result := False;
...
function TDlmErrors.Count(): Integer;
begin
  Result := FItems.Count;
end;

function TDlmErrors.ErrorCount(): Integer;
begin
  Result := CountErrors();
end;

function TDlmErrors.WarningCount(): Integer;
var
  LError: TDlmError;
begin
  Result := 0;
...
function TDlmErrors.ReachedMaxErrors(): Boolean;
begin
  Result := CountErrors() >= FMaxErrors;
end;

procedure TDlmErrors.Clear();
begin
  FItems.Clear();
end;

procedure TDlmErrors.TruncateTo(const ACount: Integer);
begin
  while FItems.Count > ACount do
    FItems.Delete(FItems.Count - 1);
end;

function TDlmErrors.GetItems(): TList<TDlmError>;
begin
  Result := FItems;
end;

function TDlmErrors.GetMaxErrors(): Integer;
begin
  Result := FMaxErrors;
end;

procedure TDlmErrors.SetMaxErrors(const AMaxErrors: Integer);
begin
  FMaxErrors := AMaxErrors;
end;

function TDlmErrors.ToString(): string;
var
  LBuilder: TStringBuilder;
  LI: Integer;
begin
...
constructor TDlmBaseObject.Create();
begin
  inherited;
  FErrors := TDlmErrors.Create();
  FOwnsErrors := True;
end;

destructor TDlmBaseObject.Destroy();
begin
...
function TDlmBaseObject.Dump(const AId: Integer): string;
begin
  Result := '';
end;

procedure TDlmBaseObject.InitConfig();
begin
end;

procedure TDlmBaseObject.LoadConfig(const AFilename: string);
begin
end;

procedure TDlmBaseObject.SaveConfig(const AFilename: string);
begin
end;


procedure TDlmBaseObject.Status(const AText: string);
begin
  if FStatusCallback.IsAssigned() then
    FStatusCallback.Callback(AText, FStatusCallback.UserData);
end;

procedure TDlmBaseObject.Status(const AText: string; const AArgs: array of const);
begin
  Status(Format(AText, AArgs));
end;

function TDlmBaseObject.GetStatusCallback(): TDlmStatusCallback;
begin
  Result := FStatusCallback.Callback;
end;

procedure TDlmBaseObject.SetStatusCallback(const ACallback: TDlmStatusCallback; const AUserData: Pointer);
begin
  FStatusCallback.Callback := ACallback;
  FStatusCallback.UserData := AUserData;
end;

procedure TDlmBaseObject.SetErrors(const AErrors: TDlmErrors);
begin
...
function TDlmBaseObject.GetErrors(): TDlmErrors;
begin
  Result := FErrors;
end;

procedure TDlmBaseObject.PropagateErrors(const AChild: TDlmBaseObject);
begin
...
constructor TDlmCommandBuilder.Create();
begin
  inherited;
...
destructor TDlmCommandBuilder.Destroy();
begin
  FreeAndNil(FParams);
...
procedure TDlmCommandBuilder.Clear();
begin
  FParams.Clear();
end;

procedure TDlmCommandBuilder.AddParam(const AParam: string);
begin
  if AParam <> '' then
    FParams.Add(AParam);
end;

procedure TDlmCommandBuilder.AddParam(const AFlag, AValue: string);
begin
  if AFlag <> '' then
  begin
    if AValue <> '' then
...
procedure TDlmCommandBuilder.AddQuotedParam(const AFlag, AValue: string);
begin
  if AValue = '' then
    Exit;
...
procedure TDlmCommandBuilder.AddQuotedParam(const AValue: string);
begin
  AddQuotedParam('', AValue);
end;

procedure TDlmCommandBuilder.AddFlag(const AFlag: string);
begin
  if AFlag <> '' then
    FParams.Add(AFlag);
end;

function TDlmCommandBuilder.Dump(const AId: Integer): string;
var
  LI: Integer;
begin
...
function TDlmCommandBuilder.GetParamCount(): Integer;
begin
...
begin
  ...
  TDlmUtils.InitConsole();
  {$IFDEF VPR_LEAK_TRACK}
  TDlmBaseObject.InitLeakTracking();
  {$ENDIF}
end;
...
begin
  {$IFDEF VPR_LEAK_TRACK}
  TDlmBaseObject.FinalizeLeakTracking();
  {$ENDIF}
end;
